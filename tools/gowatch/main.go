...
var (
	...
	flag_build           = flag.String("build", "", "Command to rebuild after changes")
...
func main() {
	...
	flag.Var(&flag_include_dirs, "include-dir", "More directories to watch")
	...
	if flag_include_dirs == nil {
		flag_include_dirs = make(globList, 0)
	}
	dirs := append(flag_include_dirs, *flag_directory)
	...
	for {
		...
		case ev := <-watcher.Events:
			if ev.Op&fsnotify.Remove == fsnotify.Remove || ev.Op&fsnotify.Write == fsnotify.Write || ev.Op&fsnotify.Create == fsnotify.Create {
				...
				if !flag_excludedDirs.Matches(ev.Name) {
					watcher.Add(ev.Name)
				}
